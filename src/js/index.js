$(function () {
  ...
  // 学校显示部分
  $("#allSchool .tab-cont").each(function () {
    var that = this;
    // 显示6个
    $(that).children('.cont-item').filter(":lt(5)").show().end().filter(":gt(5)").hide();
    $(that).children('.more').click(function () {
      $(that).children('.cont-item').show();
    });
  });
});
